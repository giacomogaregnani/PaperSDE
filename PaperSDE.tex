...
\end{equation}
i.e., $\widetilde{\mathcal M}_\epl$ is the limit for $T \to \infty$ of the matrix $\widetilde M$ by the ergodic theorem, and $\widetilde{\mathcal M}_0$ is the limit for $\epl \to 0$ of the matrix $\widetilde{\mathcal M}_\epl$ due to Lemma \ref{lem:convMeasure}. For completeness, we introduce here the symmetric matrices $\mathcal M_\epl$ and $\mathcal M_0$ which are defined as
...
\begin{theorem}\label{thm:mainTheorem} Let the assumptions of Lemma \ref{lem:ergodicity} and Lemma \ref{lem:convMeasure} hold, and let $\widehat A^\epl_k(T)$ be defined in \eqref{eq:AHatMixed} \corr{with $\delta = \mathcal O(\epl^0)$}. If $\widetilde M$ is invertible, then
	\begin{equation}
...
\begin{theorem}\label{thm:mainTheoremTilde} Let the assumptions of Theorem \ref{thm:mainTheorem_zeta} hold. Then, if \corr{$\delta = \epl^\zeta$, with $\zeta \in (0, 2)$}, it holds
	\begin{equation}
...
\end{proof}
\corr{
In light of the proof of Theorem \ref{thm:BayesianBias}, the result above guarantees that the mean of the posterior distribution $\widetilde \mu_{T, \epl}$ converges to the drift coefficient of the homogenized equation. Being the covariance matrix the same for $\mu_{T, \epl}$ and $\widetilde \mu_{T, \epl}$, it is possible to prove a positive convergence result for $\widetilde \mu_{T, \epl}$, which is given by the following Theorem.
\begin{theorem} Let the Assumptions of Theorem \ref{thm:mainTheoremTilde} hold. Then, the modified posterior measure $\widetilde\mu_{T,\epl}(\cdotp\mid X^\epl) = \mathcal N(\widetilde m_{T,\epl}, C_{T,\epl})$ satisfies
	\begin{equation}
...
\begin{theorem}\label{thm:mainTheorem_zetaAlpha} Let the assumptions of Lemma \ref{lem:ergodicity} and Assmuption \ref{as:regularityZeta} hold. Let $\widehat A^\epl_k(T)$ be defined in \eqref{eq:AHatMixed} and $\delta = \epl^\zeta$ with $\zeta > 2$. If $\widetilde M$ is invertible, then
	\begin{equation}
	\lim_{\epl \to 0} \lim_{T \to \infty} \widehat A_k^\epl(T) = \alpha, \quad \text{in probability},
	\end{equation}
	where $\alpha$ is the drift coefficient of the multiscale equation \ref{eq:SDE_MS}.
\end{theorem}
\begin{proof}
	First, let us remark that by equation \ref{eq:SDE_MS} we have for all $0 \le s < t$
	\begin{equation}
		X_t^\epl - X_s^\epl = -\alpha \int_s^t V'(X_r^\epl) \dd r - \frac1\epl \int_s^t p' \left( \frac{X_r^\epl}{\epl} \right) \dd r + \sqrt{2\sigma}(W_t - W_s),
	\end{equation}
	and applying Jensen's inequality and due to the assumptions on $p$ and the fact that $X_t^\epl$ has bounded moments of all orders (see \cite[Corollary 5.4]{PaS07}), it holds for any $p \ge 1$
	\begin{equation} \label{eq:Xt_Xs}
		\E^{\phi^\epl} \abs{X_t^\epl - X_s^\epl}^p \le C \left( (t-s)^p + (t-s)^p\epl^{-p} + (t-s)^{p/2} \right),
	\end{equation}
	where $\phi^\epl$ is the invariant measure of $X^\epl$ and $C>0$ is a constant independent of $s$, $t$ and $\epl$. By definition of $Z_t^\epl$ we have
	\begin{equation}
		X_t^\epl - Z_t^\epl = \int_0^t k(t-s) (X_t^\epl - X_s^\epl) \dd s + e^{-t/\delta}X_t^\epl,
	\end{equation}
	therefore, by \eqref{eq:Xt_Xs} and Lemmas \ref{lem:FilterJensen} and \ref{lem:FilterPoly}, we deduce
	\begin{equation}
	\begin{aligned}
	...
	\end{aligned}
	\end{equation}
	Let us recall that this result holds for $X_t^\epl$ being at stationarity and for $Z_t^\epl$ being its filtered process, and not for a couple $(X^\epl, Z^\epl)^\top \sim \mu^\epl$. Moreover, due to Lemma \ref{lem:ergodicity} we have for all $t \geq 0$ and for a constant $\widetilde C$ depending on the initial condition and $p$  
	\begin{equation}
		\E^{\rho^\epl} \abs{X^\epl - Z^\epl}^p \leq \E^{\phi^\epl}\abs{X_t^\epl - Z_t^\epl}^p + \widetilde Ce^{-\lambda t},
	\end{equation}
	which, for $t$ sufficiently big, yields
	\begin{equation} \label{eq:Xt_Zt}
		\left( \E^{\rho^\epl} \abs{X^\epl - Z^\epl}^p \right)^{1/p} \le C \left( \delta + \delta \epl^{-1} + \delta^{1/2} \right).
	\end{equation}
	Let us now consider decomposition \eqref{eq:alphaDecomposition}
	\begin{equation}
	...
	\end{equation}
	Then, by assumptions on $V$ we can write
	\begin{equation}
	V'(Z^\epl) = V'(X^\epl) + V''(\widetilde X^\epl) (Z^\epl - X^\epl),
	\end{equation}
	where $\widetilde X^\epl$ is a random variable which assumes values between $X^\epl$ and $Z^\epl$. Therefore we have
	\begin{equation}
	\begin{aligned}
	...
	\end{aligned}
	\end{equation}
	and in order to conclude we need to show that $J_1^\epl$ and $J_2^\epl$ vanish as $\epl$ goes to zero. Integrating by parts we have
	\begin{equation}
	\begin{aligned}
	...
	\end{aligned}
	\end{equation}
	and passing to the limit as $\epl \to 0$ and integrating again by parts we obtain
	...
	\end{equation}
	We now consider $J_2^\epl$ and applying Hölder's inequality with conjugate exponents $p$ and $q$ and due to the assumptions on $p$ and $V$ we get
	\begin{equation}
	\abs{J_2^\epl} \le C \epl^{-1} \left( \E^{\rho^\epl} \abs{\widetilde X^\epl}^q \right)^{1/q} \left( \E^{\rho^\epl} \abs{Z^\epl - X^\epl}^p \right)^{1/p}.
	\end{equation}
	Since $\widetilde X^\epl$ assumes values between $X^\epl$ and $Z^\epl$, which both have bounded moments of all order respectively by \cite[Corollary 5.4]{PaS07} and Lemma \ref{lem:bounded_momentZ}, and due to \eqref{eq:Xt_Zt} we have
	\begin{equation} \label{eq:limJ2_0}
	\abs{J_2^\epl} \le C \left( \delta \epl^{-1} + \delta \epl^{-2} + \delta^{1/2} \epl^{-1} \right).
	\end{equation}
	 Moreover, by Lemma \ref{lem:distanceMandTildeM} and by weak convergence of the invariant measure $\mu^\epl$ to $\mu^0$, we have
	\begin{equation}
	\lim_{\epl \to 0} \widetilde{\mathcal M}_\epl = \mathcal M_0,
	\end{equation}
	which, together with \eqref{eq:limJ1_0}, \eqref{eq:limJ2_0} and the fact that $\delta = \epl^\zeta$ with $\zeta > 2$, implies the desired result.
\end{proof}

...
\end{proof}
We can now prove a rough estimate of the difference between the trajectories $X_t^\epl$ and $Z_t^\epl$ when they are at stationarity.
...
The final result presented in this section is a sharper quantification of the difference between $X^\epl_t$ and $Z^\epl_t$ than Lemma \ref{lem:distanceZandX}. In particular, denoting $Y_t^\epl \defeq X_t^\epl/\epl$, we will make use of the decomposition
...
\begin{lemma} \label{lem:zeta} Let Assumption \ref{as:regularity} hold and $\epl,\delta>0$ be sufficiently small. Then, it holds for every $t > 0$ sufficiently big 
	...
	\end{equation}
	where $\Phi$ is the solution of the cell problem \eqref{eq:CellProblem}, $W_s$ is the Brownian motion appearing in \eqref{eq:SDE_MS} and $Y_t^\epl = X_t^\epl / \epl$. Moreover, the remainder $R(\epl,\delta)$ satisfies for every $p > 0$ the estimate
	\begin{equation} \label{eq:remainder_zeta}
	\left( \E^{\rho_\epl} \left| R(\epl,\delta) \right|^p \right)^{1/p} \le C \left( \delta + \epl^2 + \epl \delta^{1/2} \right),
	\end{equation}
...
\begin{proof} Recall that by definition of $Z_t^\epl$ we have
	...
	\end{equation}
	in law, where the remainder $R_1^\epl$ satisfies 
	...
	\begin{equation}
		Q_t \defeq \int_0^t \left(1 + \Phi'(Y_r^\epl)\right) \dd W_r,
	\end{equation}
	and therefore rewrite
	\begin{equation}
		I_2^\epl(t) = \sqrt{2\sigma} Q_t \int_0^t k(t-s)(Q_t - Q_s) \dd s.
	\end{equation}
	An application of the Itô formula to $u(s, Q_s)$ where $u(s, x) = k(t-s)x$ yields
	\begin{equation}
	\begin{aligned}
		I_2^\epl(t) &= \sqrt{2\sigma} \left(Q_t\int_0^t k(t-s) \dd s - Q_t + \delta \int_0^t k(t-s)\left(1 + \Phi'(Y_s^\epl)\right) \dd W_s\right)\\
		  			&= \delta \beta_t^\epl - \sqrt{2\sigma} e^{-t/\delta} Q_t \eqdef \delta \beta_t^\epl - R_2^\epl(t).
	\end{aligned}
	\end{equation}
	where $\beta_t^\epl$ is defined in \eqref{eq:defBeta}. For the remainder $R_2^\epl(t)$, let us remark that for all $p \geq 1$ it holds
	\begin{equation}
	\begin{aligned}
		\left(\E\abs{Q_t}^p\right)^2 \leq \E\abs{Q_t}^{2p} \leq Ct^{p-1}\int_0^t \E \abs{1 + \Phi'(Y_r^\epl)}^{2p} \dd r \leq C t
	\end{aligned}
	\end{equation}
	...
	\begin{equation}
		\E\abs{R_2^\epl(t)}^p \leq C \sqrt{t}e^{-pt/\delta}.
	\end{equation}
...
\corr{
\begin{remark} Let us remark that one can show for all $p \geq 1$ and a constant $C > 0$ independent of $\delta$
	\begin{equation}
		\left(\E\abs{\beta_t^\epl}^p \right)^{1/p} \leq C \delta^{-1/2}.
	\end{equation}
	Therefore, the estimate presented in Lemma \ref{lem:zeta} is consistent with the rough estimate of Lemma \ref{lem:distanceZandX}, and the term highlighted in the statement of Lemma \ref{lem:zeta} is indeed the dominating term in terms of powers of $\delta$.
\end{remark}
}
...
\begin{proof}[Proof of Theorem \ref{thm:mainTheorem_zeta}]
	...
	\begin{equation}
	\begin{aligned}
		J_1^\epl(t) &= 2\Sigma\delta \E \left[ \left( \int_0^t k(t-s) \dd B_s \right)^2 V''(Z_t^\epl) \right], \\
		J_2^\epl(t) &= 2\sqrt{2\Sigma} \E \left[ \left( \int_0^t k(t-s) \dd B_s \right) R(\epl,\delta) V''(Z_t^\epl) \right], \\
	...
	\begin{equation}
	J_1^\epl(t) = 2\Sigma\delta \left( \int_0^t k(t-s)^2 \dd s \right) \E [V''(Z_t^\epl)] = \Sigma \E [V''(Z_t^\epl)],
	\end{equation}
...
\begin{proof}[Proof of Theorem \ref{thm:mainTheoremTilde}] We first consider the difference between the two estimators $\widetilde A^\epl_k(T)$ and $\widehat A^\epl_k(T)$. In particular, the ergodic theorem and an algebraic equality imply
	...
	\end{aligned}
	\end{equation}
	almost surely. Therefore, due to Assumption \ref{as:regularity} which allows controlling the norm of $\mathcal M_\epl^{-1}$ and due to Lemma \ref{lem:distanceMandTildeM} we have for a constant $C > 0$

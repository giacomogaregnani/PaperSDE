...
\begin{proof}[Proof of Lemma \ref{lem:FPMarginal}] Integrating equation \eqref{eq:FPsystem} with respect to $z$ we obtain the stationary Fokker--Planck equation for the process $X^\epl$, i.e.
	...
	& + 2 \psi^\epl(z) z - \psi^\epl \int_{\R} x^2 \phi^\epl(x) \partial_z R^\epl(x, z) \dd x.
	\end{aligned}
	\end{equation}
...
\begin{lemma}
	\begin{equation}
		Z_t^\epl - X_t^\epl = \delta (A \cdot V'(X_t^\epl)) + \ldots
	\end{equation}
\end{lemma}
\begin{proof} Let $t > s$, $\Phi$ solution of \eqref{eq:CellProblem} and $Y_t^\epl = X_t^\epl / \epl$. Then, an application of the Itô formula yields \cite[Formula 5.8]{PaS07}
	...
	Recall that we have
	\begin{equation}
		X_t^\epl - Z_t^\epl = \int_0^t k(t-s) (X_t - X_s) \dd s + e^{-t/\delta}X_t.
	\end{equation}
	...
	\begin{equation}
	\begin{aligned}
		\E\abs{\widetilde R_1^\epl(t)}^{2p} &= (1-e^{-t/\delta})^{2p}\E\abs{\int_0^t R_1^\epl(t-s) \kappa(\d s)}^{2p}\\
		&\leq (1-e^{-t/\delta})^{2p} \int_0^t \E \abs{R_1^\epl(t-s)}^{2p} \kappa(\d s)\\
		&\leq C (1-e^{-t/\delta})^{2p-1} \int_0^t k(t-s)(\epl^{4p} + \epl(t-s)^p+(t-s)^{3p}) \dd s\\
		&\leq C \left( \epl^{4p} + \epl \delta^p + \delta^{3p} \right).
	\end{aligned}
	\end{equation}
	We now consider the second term. Let us remark that it holds \cite[Proposition 5.7, Section 6.2]{PaS07}
	\begin{equation}
		\sqrt{2\sigma} \int_s^t \left(1+\Phi'(Y_s^\epl)\right) \dd W_r = \sqrt{2\Sigma}\left(\widehat W_t - \widehat W_s\right) + R_2^\epl(t-s),
	\end{equation}
	in law, where $\Sigma$ is the diffusion coefficient of the homogenized equation \eqref{eq:SDE_HOM}, $\widehat W_t$ is a standard Brownian motion and 
	\begin{equation}
		\left(\E\abs{R_2^\epl(t-s)}^p\right)^{1/p} \leq C\epl^\iota,
	\end{equation}
	for all $\iota \in (0, 1/2)$. Therefore, it holds in law
	\begin{equation}
		I_2^\epl(t) = \int_0^t k(t-s) \left(\widehat W_t - \widehat W_s\right) \dd s + \widetilde R_2^\epl(t),
	\end{equation}
	where
	\begin{equation}
		\widetilde R_2^\epl(t) \defeq \int_0^t k(t-s) R_2^\epl(t-s) \dd s.
	\end{equation}
	Let us now remark that applying Jensen's inequality with $\kappa(\d s)$, due to the moments of the Gaussian distribution and applying \eqref{eq:FilterTminusS}, we have for a constant $C > 0$ depending only on $p$
	\begin{equation}
	\begin{aligned}
		\E\abs{\int_0^t k(t-s) \left(\widehat W_t - \widehat W_s\right) \dd s}^p &\leq (1-e^{-t/\delta})^{p-1} \int_0^t (2\Sigma)^{p/2}k(t-s)\E\abs{\widehat W_t - \widehat W_s}^p \dd s\\
		&\leq C \int_0^t k(t - s) (t-s)^p \dd s \\
		&\leq C \delta^p.
	\end{aligned}
	\end{equation}
	Moreover, always applying Jensen's inequality with $\kappa(\d s)$ we simply obtain
	\begin{equation}
		\E\abs{\widetilde R_2^\epl(t)}^p \leq C \epl^{p\iota}.
	\end{equation}
	...
	\begin{equation}
		\E\abs{I_4^\epl(t)}^p \leq C e^{-pt/\delta},
	\end{equation}

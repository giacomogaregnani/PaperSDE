...
\begin{proof} Recall that by definition of $Z_t^\epl$ we have
	...
	\end{equation}
	In order to obtain the bound \eqref{eq:beta} on $\beta_t^\epl$, let us remark that from \eqref{eq:rewriteI2} it holds for a constant $C > 0$ depending only on $p$ 
	\begin{equation}
	...
	\end{equation}
	which proves the bound \eqref{eq:beta}.	Let us now consider $I_3^\epl(t)$. Since $\Phi$ is bounded, we simply have
	\begin{equation}
...
\begin{proof}
	...
	\end{aligned}
	\end{equation}
	We now consider the remainder and, applying Hölder's inequality, Lemma \ref{lem:distanceZandX}, Lemma \corr{boundedness of moments of $Z^\epl$} and \eqref{eq:estimate_beta}, we get for $p,q,r$ such that $1/p+1/q+1/r=1$
	\begin{equation}
...
\begin{lemma} \label{lem:quartet_convergence}
	Let $\delta = \epl^\zeta$ with \corr{$\zeta \in (0,2)$}, then it holds
	\begin{equation}
...
\begin{proof}
	...
	\end{aligned}
	\end{equation}
	We first study $\Delta_1(\epl)$ and due to the boundedness of $\Phi'$, Assumption \ref{as:regularity} and Lemma \ref{lem:distanceZandX} we have
	\begin{equation}
	\Delta_1(\epl) \le C \E^{\eta^\epl} \abs{X^\epl - Z^\epl} \le C (\delta^{1/2} + \epl) = C (\epl^\zeta + \epl),
	\end{equation}
	which implies
	\begin{equation}
	\lim_{\epl \to 0} \Delta_1(\epl) = 0.
	\end{equation}
	We now consider $\Delta_2(\epl)$. Integrating equation \ref{eq:FPSDEquartet} with respect to $z$ and $b$ we obtain the Fokker-Planck equation for the stationary marginal distribution $\lambda \colon \R \times [0,L]$, $\lambda = \lambda(x,y)$, of the couple $(X^\epl,Y^\epl)$
	\begin{equation}
	\begin{aligned}

...
\begin{theorem}\label{thm:mainTheorem_zeta} Let the assumptions of Lemma \ref{lem:ergodicity} and Lemma \ref{lem:convMeasure} hold, let $\widehat A^\epl_k(T)$ be defined in \eqref{eq:AHatMixed} and let $\delta = \epl^\zeta$ with $\zeta \in (0,1)$. If $\widetilde M$ is invertible, then
		\begin{equation}
...
\begin{lemma} \label{lem:zeta}
	For $\epl,\delta>0$ sufficiently small and $t>0$ sufficiently big, there exists a standard Brownian motion $B_t$ independent of the processes $X_t$ and $Z_t$ such that
	\begin{equation}
	X_t^\epl - Z_t^\epl = \sqrt{2\Sigma}\delta \int_0^t k(t-s) \dd B_s + R(\epl,\delta),
	\end{equation}
	in law. The remainder $R(\epl,\delta)$ satisfies, for every $p > 0$ and $\iota \in (0,1/2)$, the estimate
	\begin{equation} \label{eq:remainder_zeta}
		\left( \E^{\rho_\epl} \left| R(\epl,\delta) \right|^p \right)^{1/p} \le C \left( \delta + \epl^\iota + \epl \delta^{1/2} \right),
	\end{equation}
	where $C$ is independent of $\epl$ and $\delta$.
\end{lemma}
\begin{proof} Let $t > s$, $\Phi$ solution of \eqref{eq:CellProblem} and $Y_t^\epl = X_t^\epl / \epl$. Then, an application of the Itô formula yields \cite[Formula 5.8]{PaS07}
	...
	\end{equation}
	Plugging \eqref{eq:App_DiffXtXs} into the equation above, we obtain
	\begin{equation}
	...
	\begin{aligned}
	I_1^\epl(t) &= -(A \cdot V'(X_t^\epl)) \int_0^t k(t-s) (t-s)\dd s + \int_0^t k(t-s)R^\epl(t-s) \dd s\\
	...
	\end{equation}
	Now, Jensen's inequality with $\kappa(\d s)$ and the inequalities \eqref{eq:FirstTermPavStu} and \eqref{eq:FilterTminusS} yield for all integers $p \geq 1$ and a constant $C$ depending only on $p$
	\begin{equation}
	\begin{aligned}
	\E^{\phi^\epl}\abs{\widetilde R_1^\epl(t)}^{2p} &= (1-e^{-t/\delta})^{2p}\E^{\phi^\epl}\abs{\int_0^t R_1^\epl(t-s) \kappa(\d s)}^{2p}\\
	&\leq (1-e^{-t/\delta})^{2p} \int_0^t \E^{\phi^\epl} \abs{R_1^\epl(t-s)}^{2p} \kappa(\d s)\\
	&\leq C (1-e^{-t/\delta})^{2p-1} \int_0^t k(t-s)(\epl^{4p} + \epl^{2p}(t-s)^p+(t-s)^{3p}) \dd s\\
	&\leq C \left( \epl^{4p} + \epl^{2p} \delta^p + \delta^{3p} \right).
	\end{aligned}
	\end{equation}
	Therefore, for $t$ sufficiently big, we get
	\begin{equation}
	\begin{aligned}
		\left(\E^{\phi^\epl}\abs{I_1^\epl(t)}^{p}\right)^{1/p} \le& C \left( \delta + \epl^2 + \epl \delta^{1/2} + \delta^{3/2} \right) \\
		\le& C \left( \delta + \epl^2 + \epl \delta^{1/2} \right).
	\end{aligned}
	\end{equation}
	We now consider the second term. Let us remark that it holds \cite[Proposition 5.7, Section 6.2]{PaS07}
	\begin{equation}
	\sqrt{2\sigma} \int_s^t \left(1+\Phi'(Y_s^\epl)\right) \dd W_r = \sqrt{2\Sigma}\left( B_t - B_s\right) + R_2^\epl(t-s),
	\end{equation}
	in law, where $\Sigma$ is the diffusion coefficient of the homogenized equation \eqref{eq:SDE_HOM}, $B_t$ is a standard Brownian motion \corr{independent of the process $X_t$ (?)} and 
	\begin{equation}
	\left(\E^{\phi^\epl}\abs{R_2^\epl(t-s)}^p\right)^{1/p} \leq C\epl^\iota,
	\end{equation}
	for all $\iota \in (0, 1/2)$. Therefore, it holds in law
	\begin{equation} \label{eq:I2_decomposition}
	I_2^\epl(t) = \sqrt{2\Sigma} \int_0^t k(t-s) \left(B_t - B_s\right) \dd s + \widetilde R_2^\epl(t),
	\end{equation}
	where
	\begin{equation}
	\widetilde R_2^\epl(t) \defeq \sqrt{2\Sigma} \int_0^t k(t-s) R_2^\epl(t-s) \dd s.
	\end{equation}
	Applying Jensen's inequality with $\kappa(\d s)$ we obtain
	\begin{equation}
	\left(\E^{\phi^\epl} \abs{\widetilde R_2^\epl(t)}^p\right)^{1/p} \leq C \epl^{\iota}.
	\end{equation}
	Let us remark that an application of the Itô formula yields
	\begin{equation}
		\dd \left( k(t-s)B_s \right) = \frac1\delta k(t-s)B_s \dd s + k(t-s) \dd B_s,
	\end{equation}
	which implies
	\begin{equation} \label{eq:Ito_kB}
		\int_0^t k(t-s)B_s \dd s = B_t - \delta \int_0^t k(t-s) \dd B_s.
	\end{equation}
	Substituting \eqref{eq:Ito_kB} into \eqref{eq:I2_decomposition} and recalling that
	\begin{equation}
		\int_0^t k(t-s) \dd s = 1 - e^{-t/\delta},
	\end{equation}
	we obtain
	\begin{equation}
		I_2^\epl(t) = \sqrt{2\Sigma} \delta \int_0^t k(t-s) \dd B_s - \sqrt{2\Sigma} e^{-t/\delta} B_t.
	\end{equation}
	Moreover note that
	\begin{equation}
			\left(\E^{\phi^\epl} \abs{\sqrt{2\Sigma} e^{-t/\delta} B_t}^p\right)^{1/p} \le C e^{-t/\delta}.
	\end{equation}
...
\begin{proof}[Proof of Theorem \ref{thm:mainTheorem_zeta}]
	...
	\end{equation}
	Then following the proof of Theorem \ref{thm:mainTheorem} we only need to show that if $\delta = \epl^\zeta$ with $\zeta \in (0,1)$ we have
	\begin{equation}
		\lim_{\epl \to 0} \mathcal A^\epl(\delta) = A, \quad \text{in law}.
	\end{equation}
	Using Lemma \ref{lem:zeta} and geometric ergodicity (Lemma \ref{lem:ergodicity}), we have the following equality in law
	\begin{equation}
	\begin{aligned}
		\mathcal A^\epl(\delta) &= \widetilde{\mathcal M}_\epl^{-1} \frac1\delta \lim_{t \to \infty} \E [(X_t^\epl - Z_t^\epl)^2 V''(Z_t^\epl)] \\
		&= \widetilde{\mathcal M}_\epl^{-1} \frac1\delta \lim_{t \to \infty} \E \left[ \left( \sqrt{2\Sigma}\delta \int_0^t k(t-s) \dd B_s + R(\epl,\delta) \right)^2 V''(Z_t^\epl) \right] \\
	...
	\begin{equation}
	\begin{aligned}
		J_1^\epl(t) &= 2\Sigma\delta \E^{\rho^\epl} \left[ \left( \int_0^t k(t-s) \dd B_s \right)^2 V''(Z_t^\epl) \right], \\
		J_2^\epl(t) &= 2\sqrt{2\Sigma} \E^{\rho^\epl} \left[ \left( \int_0^t k(t-s) \dd B_s \right) R(\epl,\delta) V''(Z_t^\epl) \right], \\
		J_3^\epl(t) &= \frac1\delta \E^{\rho^\epl} \left[ R(\epl,\delta)^2 V''(Z_t^\epl) \right].
	\end{aligned}
	\end{equation}
	Let us consider the three terms separately. First, since the Brownian motion $B_s$ is independent of the process $Z_t^\epl$, applying Itô isometry we get
	\begin{equation}
	J_1^\epl(t) = 2\Sigma\delta \left( \int_0^t k(t-s)^2 \dd s \right) \E [V''(Z_t^\epl)] = \Sigma \E^{\rho^\epl} [V''(Z_t^\epl)],
	\end{equation}
	and, by Lemma \ref{lem:distanceZandX} and weak convergence of the invariant measure, we obtain
	\begin{equation}
	\lim_{\epl \to 0} \lim_{t \to \infty} J_1^\epl(t) = \Sigma \lim_{\epl \to 0} \E^{\rho^\epl} [V''(Z^\epl)] = \Sigma \E^{\rho^0} [V''(X)].
	\end{equation}
	Moreover, due to \eqref{eq:remainder_zeta}, we have the following bounds for $J_2^\epl(t)$ and $J_3^\epl(t)$
	\begin{equation}
	\begin{aligned}
		\abs{J_2^\epl(t)} &\le C \left( \delta^{1/2} + \epl^\iota \delta^{-1/2} + \epl \right), \\
		\abs{J_3^\epl(t)} &\le C \left( \delta + \epl^{2\iota} \delta^{-1} + \epl^2 \right).
	\end{aligned}
	\end{equation}
...
\begin{proof} Let us first remark that the filtering kernel satisfies
	...
	\begin{equation}\label{eq:FilterTminusS}

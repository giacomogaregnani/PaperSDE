...
\begin{proof} Let us introduce the notation
	...
	\end{equation}
	Let us consider $J_{1,1}^\epl(T)$ and denote by $\Phi_1^\epl$ the solution of \eqref{eq:CLT_Poisson} with 
	\begin{equation}
		\chi(x, z) = 
	\end{equation}
...
%	\begin{equation}
%	\begin{aligned}
%		\sqrt{T}\left(\alpha - A + I_1^\epl(T)\right) = & \sqrt{T}\left(\alpha - A + \widetilde M^{-1}\E^{\rho^\epl}\left[V'(Z^\epl)\frac1\epl p'\left(\frac{X^\epl}{\epl}\right)\right]\right)\\
%		& + \frac{\widetilde M^{-1} R_T^\epl}{\sqrt{T}} - \sqrt{2\sigma} \frac{\widetilde M^{-1} S_T^\epl}{\sqrt{T}}.
%	\end{aligned}
%	\end{equation}
	\begin{equation}
		J_{1,1}^\epl(T)= \sqrt{T} \mathcal M_\epl^{-1} \left( \frac{R_T^\epl}{T} - \sqrt{2\sigma}\frac{S^\epl_T}{T} \right)
	\end{equation}
	Since \corr{$R^\epl_T$ is bounded independently of $\epl$}, we first get
%	\begin{equation}
%		\lim_{T \to \infty} \frac{\widetilde M^{-1} R_T^\epl}{\sqrt{T}} = 0, \quad \text{a.s.}
%	\end{equation}
	\begin{equation}
		\lim_{T \to \infty} \frac{\mathcal M_\epl^{-1} R_T^\epl}{\sqrt{T}} = 0, \quad \text{a.s.}
	\end{equation}
	Repeating the same reasoning as for $Q_T^\epl$, we get 
%	\begin{equation}
%		\lim_{T \to \infty} \sqrt{2\sigma} \frac{\widetilde M^{-1} S_T^\epl}{\sqrt{T}} = \Lambda_1^\epl \sim \mathcal N(0, \Gamma_1^\epl),
%	\end{equation}
	\begin{equation}
	\lim_{T \to \infty} \sqrt{2\sigma} \frac{\mathcal M_\epl^{-1} S_T^\epl}{\sqrt{T}} = \Lambda_1^\epl \sim \mathcal N(0, \Gamma_1^\epl),
	\end{equation}
	...
	\end{equation}
	\corr{For the second part of \eqref{eq:decomposition_first_term} we follow a similar procedure defining a different Poisson equation. The argument has to be slightly modified because we have to work with random matrix rather than random vector, but finally we obtain a second Gaussian term $\mathcal N(0, \Gamma_2^\epl)$. Therefore, we have to study the limit as $\epl$ goes to zero of the covariance matrices $\Gamma_1^\epl$ and $\Gamma_2^\epl$, which depend on the solutions $\Phi_1^\epl$ and $\Phi_2^\epl$ of the Poisson problems.} \\
	
	\corr{Idea for the second term:} The second term can be decomposed in 
	\begin{equation}
	\begin{aligned}
	...
	\end{aligned}
	\end{equation}
	\corr{and we need the rate of convergence of the measure $\rho^\epl$ to $\rho^0$.}
\end{proof}

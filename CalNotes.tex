...
\begin{lemma}\label{lem:CLT} Let $\mathcal L_\epl$ be the generator of the couple $(X^\epl, Z^\epl)^\top$, i.e.,
	...
	\begin{equation}\label{eq:CLT_Poisson}
		-\mathcal L_\epl \Phi^\epl = \chi^\epl -  \E^{\rho^\epl}[\chi^\epl(X^\epl, Z^\epl)],
	\end{equation}
	for $\chi^\epl \colon \R^2 \to \R^N$ satisfying $\E^{\rho^\epl}[\Phi^\epl(X^\epl, Z^\epl)] = 0$. Then, it holds
	...
	\begin{equation}\label{eq:CLT_Equiv}
		...
		\begin{aligned}[t]
		[&\chi^\epl(X^\epl, Z^\epl) \otimes \Phi^\epl(X^\epl, Z^\epl) \\
		&+ \Phi^\epl(X^\epl, Z^\epl) \otimes \chi^\epl(X^\epl, Z^\epl)].
		\end{aligned}
	\end{equation}	
\end{lemma}
\begin{proof} The proof of \eqref{eq:CLT_PoissonIto} and \eqref{eq:CLT_PoissonRemainders} is an application of the Itô formula (see e.g. \cite[Remark 6.17]{PaS08}). For \eqref{eq:CLT_Equiv}, it is possible to show that since $\mathcal L_\epl^*\rho^\epl = 0$ it holds 
	\begin{equation}
		\mathcal L_\epl^* (\Phi^\epl \rho^\epl) = 2 \sigma \rho^\epl \partial_{xx}^2 \Phi^\epl - \rho^\epl \mathcal L_\epl\Phi^\epl + 2 \sigma \partial_x \Phi^\epl \partial_x \rho^\epl.
	\end{equation}
...
\begin{proof}[Proof of Theorem \ref{thm:CLT}] Let us introduce the notation
	...
	\end{aligned}
	\end{equation}
	Let us denote by $\Phi^\epl$ the solution of \eqref{eq:CLT_Poisson} in Lemma \ref{lem:CLT} with right hand side
	\begin{equation}
	\chi^\epl(x, z) = \frac 1 \epl p' \left ( \frac x \epl \right ) V'(z) - V'(z) \otimes V'(x) \mathcal M_\epl^{-1} \mathfrak p_\epl,
	\end{equation}
	and note that $\E^{\rho^\epl}[\chi^\epl(X^\epl,Z^\epl)] = 0$. Then we have
	\begin{equation}
	...
	\begin{equation}
	\begin{aligned}
	\Gamma_1^\epl &= 2\sigma \mathcal M_\epl^{-1}\E^{\rho^\epl}[\partial_x \Phi^\epl(X^\epl, Z^\epl) \otimes \partial_x \Phi^\epl(X^\epl, Z^\epl)]\mathcal M_\epl^{-\top} \\
	&= \mathcal M_\epl^{-1}\E^{\rho^\epl}[\Phi^\epl(X^\epl, Z^\epl) \otimes \chi^\epl(X^\epl, Z^\epl) + \chi^\epl(X^\epl, Z^\epl) \otimes \Phi^\epl(X^\epl, Z^\epl)]\mathcal M_\epl^{-\top}.
	\end{aligned}
	\end{equation}
	\corr{For the covariance $\Gamma_1^\epl$ we don't know how it behaves in the limit $\epl \to 0$. It would be nice if it was vanishing, but it involves the solution $\Phi^\epl$, which has r.h.s. depending of $\epl$. We noticed that the homogenized problem of
	\begin{equation}
		-\mathcal L_\epl \Phi^\epl = \chi^\epl,
	\end{equation}
	is given by
	\begin{equation}
		-\mathcal L_0 \Phi^0 = 0,
	\end{equation}
	whose zero-mean solution is $\Phi^0 = 0$.}

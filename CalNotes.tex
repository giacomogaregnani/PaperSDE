...
\begin{proof}
	...
	\end{equation}
	Therefore, since $\delta = \mathcal O(\epl^\zeta)$ for $\zeta \in (0, 2)$, and due to geometric ergodicity the terms $J_2^\epl(t)$ and $J_3^\epl(t)$ vanish in the limit for $t \to \infty$ and $\epl \to 0$. Furthermore, by Lemma \ref{lem:distanceMandTildeM} and by weak convergence of the invariant measure $\mu^\epl$ to $\mu^0$, we have
	\begin{equation}
...
\begin{theorem}\label{thm:mainTheoremTilde} Let the assumptions of Theorem \ref{thm:mainTheorem} hold. Then, if \corr{$\delta = \epl^\zeta$, with $\zeta \in (0, 2)$}, it holds
	\begin{equation}

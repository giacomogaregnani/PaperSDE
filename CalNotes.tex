...
\begin{proof} Let us consider the decomposition \eqref{eq:alphaDecomposition}, which yields
	\begin{equation}
		\sqrt{T}\left(\widehat A^\epl_k(T) - A\right) = \sqrt{T}\left(\alpha - A + I_1^\epl(T)\right) - \sqrt{T} I_2^\epl(T).
	\end{equation}
	\corr{Second term is OK:} We rewrite the term involving $I_2^\epl(T)$ as
	\begin{equation}
	...
	\corr{Idea for the first term:} Let $\mathcal L^\epl$ be the generator of the joint process $(X^\epl, Z^\epl)$, i.e.,
	\begin{equation}
	...
	\end{equation}
	Then, let $\Phi^\epl$ the solution of the Poisson equation
	\begin{equation}
		\mathcal L^\epl \Phi^\epl(x, z) = V'(z)\frac1\epl p'\left(\frac{x}{\epl}\right) - \E^{\rho^\epl}\left[V'(Z^\epl)\frac1\epl p'\left(\frac{X^\epl}{\epl}\right)\right].
	\end{equation}	
	...
	\begin{equation}
		R_T^\epl \defeq \Phi^\epl(X^\epl_T, Z^\epl_T) - \Phi^\epl(X^\epl_0, Z^\epl_0),
	\end{equation}
	and $S^\epl_T$ is the martingale defined as
	\begin{equation}
		S^\epl_T \defeq \int_0^T \partial_x \Phi^\epl(X_t^\epl, Z_t^\epl) \dd t.
	\end{equation}
	Therefore, we have
	\begin{equation}
	\begin{aligned}
	...
	\end{aligned}
	\end{equation}
	Since \corr{$R^\epl_T$ is bounded independently of $\epl$}, we first get by the ergodic theorem that
	\begin{equation}
	...
	\begin{equation}
		\Gamma_1^\epl = 2\sigma \E^{\rho^\epl}[V'(Z^\epl) \otimes V'(X^\epl)]^{-1}\E^{\rho^\epl}[\partial_x \Phi^\epl(X^\epl, Z^\epl) \otimes \partial_x \Phi^\epl(X^\epl, Z^\epl)]\E^{\rho^\epl}[V'(X^\epl) \otimes V'(Z^\epl)]^{-1}.
	\end{equation}

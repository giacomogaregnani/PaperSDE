...
\begin{figure}[t]
	\centering
	\begin{tabular}{c}
		\includegraphics[]{Figures/TrajD1} \\
		\includegraphics[]{Figures/TrajD5} \\
		\includegraphics[]{Figures/TrajD10}
	\end{tabular}
	\caption{\corr{Filtering a trajectory $X^\epl$ obtained with $V(x) = x^2 / 2$, $p(y) = \cos(y)$, $\alpha = 1$, $\sigma = 0.5$ and $\epl = 0.1$. The filtering width is $\delta = \{1, \sqrt{\epl}, \epl\}$ from top to bottom, respectively.}}
	\label{fig:filter}
\end{figure}
...
In a general case, it is not possible to find an explicit solution to \eqref{eq:FPsystem}. Nevertheless, it is possible to show some relevant properties of the solution itself, which are summarized in the following Lemma, whose proof is given in Appendix \ref{ap:Proofs}.
...
\corr{We can now prove the main result, namely the convergence of the filtering-based estimator to the drift coefficient of the homogenized equation.}
...
where 
\begin{equation}
\widetilde {\mathcal M}_\epl \defeq \E^{\rho^\epl} [ V'(Z^\epl) \otimes V'(X^\epl) ].
\end{equation}
...
\end{equation}
where the function $\rho^0$ is the density of the limit invariant distribution for $\epl \to 0$, as in Lemma \ref{lem:convMeasure}, and where
\begin{equation}
	\widetilde {\mathcal M}_0 \defeq \E^{\rho^0} [ V'(Z) \otimes V'(X) ].
\end{equation}
Due to Corollary \ref{lem:FPMarginal_Hom}, we have
...
\begin{proposition} \label{prop:zeta} Let Assumption \ref{as:regularity} hold and $\epl,\delta>0$ be sufficiently small. Then, it holds for every $t > 0$ sufficiently big 
	...
	\end{equation}
	where $\Phi$ is the solution of the cell problem \eqref{eq:CellProblem}, $W_s$ is the Brownian motion appearing in \eqref{eq:SDE_MS} and $Y_t^\epl = X_t^\epl / \epl$. Moreover, $\beta_t^\epl$ and the remainder $R(\epl,\delta)$ satisfy for every $p \geq 1$ the estimates
	\begin{equation} \label{eq:estimate_beta}
	\left( \E^{\phi^\epl} \abs{\beta_t^\epl}^p \right)^{1/p} \leq C \delta^{-1/2},
	\end{equation}
	and
...
It is clear from the Proposition above that understanding the properties of the process $\beta_t^\epl$ is key to understanding the behavior of the difference between $X^\epl$ and $Z^\epl$. In particular, it is relevant to study the asymptotic properties of $\beta_t^\epl$ for long times
...
\begin{lemma} \label{lem:quartet_convergence}
	Let $\delta = \epl^\zeta$ with $\zeta > 0$, then it holds
	\begin{equation}
...
\section{Proofs of Sections \ref{sec:ergodic}}\label{ap:Proofs}
...
Recall that by definition of $Z_t^\epl$ we have
	...
	\end{equation}
	In order to obtain the bound \eqref{eq:estimate_beta} on $\beta_t^\epl$, let us remark that from \eqref{eq:rewriteI2} it holds for a constant $C > 0$ depending only on $p$ 
	\begin{equation}
		\left(\E\abs{\beta_t^\epl}^p\right)^{1/p}\leq C \delta^{-1}\left(\E\abs{I_2^\epl(t)}^p\right)^{1/p} + C \delta^{-1} \left(\E\abs{R_2^\epl(t)}^p\right)^{1/p}.
	\end{equation}
	...
	Therefore, it holds
	\begin{equation}
		\left(\E\abs{\beta_t^\epl}^p\right)^{1/p}\leq C \delta^{-1/2},
	\end{equation}
...
\section{Proofs of Section \ref{sec:Fast}} \label{ap:zeta}
...
\begin{lemma}\label{lem:distanceMandTildeM} Let the assumptions of Corollary \ref{cor:distanceZandX} hold. Then the matrices $\mathcal M$ and $\widetilde{\mathcal M}_\epl$ defined in \eqref{eq:CalM} satisfy
	\begin{equation}
...
\subsection{Proof of Lemmas \ref{lem:quartet} and \ref{lem:quartet_convergence}}

\begin{proof}[Proof of Lemma \ref{lem:quartet}]
	Let us consider the following system of stochastic differential equations for the processes $X_t^\epl, Z_t^\epl, \beta_t^\epl, Y_t^\epl$
	\begin{equation}
	\label{eq:systemSDEquartet}
	\begin{aligned}
	...
	\d \beta_t^\epl &= - \frac1\delta \beta_t^\epl \dd t + \frac{\sqrt{2\sigma}}{\delta}(1+\Phi'(Y_t^\epl)) \dd W_t, \\
	\d Y_t^\epl &= -\frac1\epl \alpha \cdot V'(X_t^\epl) \dd t - \frac{1}{\epl^2} p'(Y_t^\epl) \dd t+ \frac{\sqrt{2\sigma}}{\epl} \dd W_t,
	\end{aligned}
	\end{equation}
	whose generator $\widetilde{\mathcal L}^\epl$ is given by
	\begin{equation}
	\begin{aligned}
	\widetilde{\mathcal L}^\epl =& -\left( \alpha \cdot V'(x) + \frac1\epl p'(y) \right) \partial_x + \frac1\delta (x-z) \partial_z - \frac1\delta b \partial_b - \left( \frac1\epl \alpha \cdot V'(x) + \frac{1}{\epl^2} p'(y) \right) \partial_y \\
	...
	\end{aligned}
	\end{equation}
	Let us denote by $\eta^\epl \colon \R^3 \times [0,L] \to \R$, $\eta^\epl = \eta^\epl(x,z,b,y)$, the invariant measure of the quartet $(X_t^\epl, Z_t^\epl, \beta_t^\epl, Y_t^\epl)$. Then $\eta^\epl$ solves the stationary Fokker-Planck equation
	\begin{equation} \label{eq:FPSDEquartet}
	\begin{aligned}
	\partial_x \left( \left( \alpha \cdot V'(x) + \frac1\epl p'(y) \right) \eta^\epl \right) - \partial_z \left( \frac1\delta (x-z) \eta^\epl \right) & \\
	+ \partial_b \left( \frac1\delta b \eta^\epl \right) + \partial_y \left( \left( \frac1\epl \alpha \cdot V'(x) + \frac{1}{\epl^2} p'(y) \right) \eta^\epl \right) & \\
	+ \sigma \left( \partial_{xx}^2 \eta^\epl + \partial_{xy}^2 \left( \frac2\epl \eta^\epl \right) + \partial_{yy}^2 \left( \frac{1}{\epl^2} \eta^\epl \right) \right) & \\
	+ \sigma \left( \partial_{xb}^2 \left( \frac{2(1+\Phi'(y))}{\delta} \eta^\epl \right) + \partial_{yb}^2 \left( \frac{2(1+\Phi'(y))}{\epl \delta} \eta^\epl \right) + \partial_{bb}^2 \left( \frac{(1+\Phi'(y))^2}{\delta^2} \eta^\epl \right) \right) & = 0.
	\end{aligned}
	\end{equation}
	We now multiply the equation above by a continuous differentiable function $f \colon \R^2 \to \R^N$, $f = f(z,b)$, and integrate with respect to $x$, $z$, $b$ and $y$. Then integration by parts yields
	\begin{equation}
	...
	which implies the following identity in $\R^N$
	\begin{equation}
	\delta \E^{\eta^\epl}\left[ \partial_b f(Z^\epl, \beta^\epl) \beta^\epl \right] = \sigma \E^{\eta^\epl} \left[ \partial_{bb}^2 f(Z^\epl, \beta^\epl) (1+\Phi'(Y^\epl)) \right] + \delta \E^{\eta^\epl} \left[ \partial_z f(Z^\epl, \beta^\epl) (X^\epl-Z^\epl) \right].
	\end{equation}
	Choosing
	\begin{equation}
	...
	\begin{equation}
	\begin{aligned}
	\delta \E^{\eta^\epl}\left[ (\beta^\epl)^2 V''(Z^\epl) \right] &= \sigma \E^{\eta^\epl} \left[ V''(Z^\epl) (1+\Phi'(Y^\epl)) \right] + \frac\delta2 \E^{\eta^\epl} \left[ (\beta^\epl)^2 V'''(Z^\epl) (X^\epl-Z^\epl) \right] \\
	&\eqdef \sigma \E^{\eta^\epl} \left[ V''(Z^\epl) (1+\Phi'(Y^\epl)) \right] + \widetilde R(\epl,\delta).
	\end{aligned}
	\end{equation}
	We now consider the remainder and, applying Hölder's inequality, Lemma \ref{cor:distanceZandX}, Lemma \ref{lem:bounded_momentZ} and \eqref{eq:estimate_beta}, we get for $p,q,r$ such that $1/p+1/q+1/r=1$
	\begin{equation}
	\left| \widetilde R(\epl,\delta) \right| \le C \delta \left( \E^{\eta^\epl} |\beta^\epl|^{2p} \right)^{1/p} \left( \E^{\eta^\epl} |V'''(Z^\epl)|^q \right)^{1/q} \left( \E^{\eta^\epl} |X^\epl - Z^\epl|^r \right)^{1/r} \le C (\delta^{1/2} + \epl),
	\end{equation}
	which completes the proof.
\end{proof}

\begin{proof}[Proof of Lemma \ref{lem:quartet_convergence}]
	Let us introduce the notation 
	\begin{equation}
	...
	\end{equation}
	We now consider $\Delta_2(\epl)$. Integrating equation \eqref{eq:FPSDEquartet} with respect to $z$ and $b$ we obtain the Fokker-Planck equation for the stationary marginal distribution $\lambda \colon \R \times [0,L]$, $\lambda = \lambda(x,y)$, of the couple $(X^\epl,Y^\epl)$
	\begin{equation}
	\begin{aligned}
	...
	\end{aligned}
	\end{equation}
	which shows that $\Delta_2(\epl) = 0$ and completes the proof.
\end{proof}

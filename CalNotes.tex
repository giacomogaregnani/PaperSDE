...
\corr{Failure without pre-processing: explain.}
...
\end{equation}
where the equality is component-wise in $\R^d$. Let $\beta, \delta > 0$ and let us consider a family of exponential kernel functions defined as
\begin{equation}
...
We will treat analytically two different cases. First, the case where the state $X_t^\epl$ is a vector of $\R^d$ and we wish to find the 
